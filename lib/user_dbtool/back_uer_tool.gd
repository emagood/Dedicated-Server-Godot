'''
CAMBIAR COSAS ROMPE COSAS , RESPALDO AQUI
respaldo de toolbin
'''
...
#var size_64bit: int
#
#func _init(filename: String, size_8bit: int, size_16bit: int, size_32bit: int, size_64bit: int):
	#self.filename = filename
	#self.size_8bit = size_8bit
	#self.size_16bit = size_16bit
	#self.size_32bit = size_32bit
	#self.size_64bit = size_64bit
	#ensure_file_exists()  # Asegura que el archivo exista al inicializar la clase
#
#func open_file():
...
#
## Clase y funciones anteriores se mantienen, solo se agregan ajustes en save_data_block y load_data_block.
...
#
	...
	#print("Tamaño total del bloque:", total_size)
#
	#var found_position = -1
	#while file.get_position() < file.get_length():
		#var initial_position = file.get_position()
		#var current_id = file.get_buffer(8)
		#print("Identificador actual encontrado en el archivo (guardando):", current_id)
#
		## Verificar si el bloque actual es el mismo o está vacío (identificador en 0)
		#if current_id == identifier or current_id == PackedByteArray([0, 0, 0, 0, 0, 0, 0, 0]):
			#found_position = initial_position
			#break
		#file.seek(initial_position + total_size)
#
	#
	#if found_position != -1:
		#file.seek(found_position)
	#else:
		#file.seek_end()
		#print("Agregando datos al final del archivo en la posición:", file.get_position())
#
...
#func _write_data_block(file, block: BinaryDataBlock):
	...
	#print("Escribiendo identificador:", block.identifier)
#
	#for value in block.data_8bit:
		#file.store_8(value)
#
...
#
	#while file.get_position() < file.get_length():
		#var initial_position = file.get_position()
		#var current_id = file.get_buffer(8)
		#print("Comparando identificador (cargando):", current_id, " con:", identifier)
...
#
			#close_file(file)
			#return BinaryDataBlock.new(current_id, data_8bit, data_16bit, data_32bit, data_64bit)
#
		#file.seek(initial_position + total_size)
#
	#close_file(file)
...
#
	#while file.get_position() < file.get_length():
		#var current_id = file.get_buffer(8)
			...
			#close_file(file)
			#return true
		#file.seek(file.get_position() + total_size - 8)  # Ajustar la posición correctamente
#
	#close_file(file)
	#return false
#
...
#var cache_size: int = 0  # Tamaño máximo de la caché (opcional)
#
...
#
			...
			#for i in range(size_64bit):
				#data_64bit.append(file.get_64())
#
			## Crear una instancia de BinaryDataBlock y añadirla a la lista
